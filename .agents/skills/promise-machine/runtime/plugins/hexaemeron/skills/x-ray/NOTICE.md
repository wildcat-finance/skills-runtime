...
- Modifications: the upstream browsing README and demo media are not
  distributed here; `SKILL.md` is the only instruction copy and describes the
  current verdict output. Added alongside the upstream files: this notice, the
  `LICENSE` copy, and `agents/openai.yaml` for Codex discovery.
